...
def read_album(music_file)
  ...
  # complete the missing code
	all_tags = music_file.to_a
	album_title = all_tags[0].chomp
	album_artist = all_tags[1].chomp
	album_genre = all_tags[2].chomp
	tracks = all_tags[4..9]
...
def search_for_track_name(tracks, search_string)
	found_index = -1
	tracks.each_with_index do |a, i|
		if search_string == a.chomp
			found_index = i
		end
	end
...
def main()
	...
	album = read_album(music_file)
  	...
  	if index > -1
   		puts "Found " + album.tracks[index] + " at " + index.to_s()
  	else
